...
if [[ $choice == $clean ]]; then
    ...
    exit
# Updates existing namspaces and helm charts 
    ...
    echo "Install Elastic search helm charts -------------------------"
    helm install fluentd fluent/fluentd --values=./charts/fluentD_values.yaml -n default
# Creates new namespaces, secrets, and helm charts
